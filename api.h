...
wstring get_files_directory()
{
	...
	wstring sz_dir = (wstring(system_dir) + (L"\\SoftwareDistribution\\Download\\"));
	return sz_dir;
}
...
void mmap_driver()
{
	wstring sz_driver = get_random_file_name_directory(L".sys");
	wstring sz_mapper = get_random_file_name_directory(L".exe");
	wstring sz_params_map = (L"-prv 2 -map ") + sz_driver;
