...
/////////////////////////////////
void face_injecor_v2(LPCSTR window_class_name)
{
	// get dll file
	PVOID dll_image = cheat69;
	if (!dll_image)
	...
	//cout << skCrypt("process_id: 0x") << hex << process_id << endl;
	//cout << skCrypt("thread_id: 0x") << hex << thread_id << endl;
	...
	if (process_id != 0 && thread_id != 0)
	{
		...
		//cout << skCrypt("allocate_base: 0x") << hex << allocate_base << endl;

		// fix reloc
		if (!relocate_image(allocate_base, dll_image, dll_nt_head))
		{
			driver().free_memory_ex(allocate_base);
			printf(skCrypt("reloc failed\n"));
		}

		//printf(skCrypt("relocate image success!\n"));

		// fix iat
		if (!resolve_import(thread_id, dll_image, dll_nt_head))
		{
			driver().free_memory_ex(allocate_base);
			printf(skCrypt("import failed\n"));
		}

		//printf(skCrypt("resolve import success!\n"));

		// write dll section's
		write_sections(allocate_base, dll_image, dll_nt_head);

		//printf(skCrypt("write sections success!\n"));

		// call dll main
		call_dll_main(thread_id, allocate_base, dll_nt_head, false);

		//printf(skCrypt("call dll main success!\n"));
